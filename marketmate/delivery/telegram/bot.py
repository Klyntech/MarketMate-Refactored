...
def _format_signal(s) -> str:
    ...
    tp3_line = (
        f"• *TP3*:  `{_fmt(s.tp3, sym)}` (liquidity)\n"
        if s.tp3 else ""
    )

    zone_mid = getattr(s, "entry_mid", (s.entry_low + s.entry_high) / 2)
    if abs(s.entry_high - s.entry_low) / max(zone_mid, 1) < 0.001:
        entry_str = f"`{_fmt(zone_mid, sym)}`"
    else:
        entry_str = f"`{_fmt(s.entry_low, sym)}` – `{_fmt(s.entry_high, sym)}`"

    return f"""
{direction_icon} *{s.direction} — {s.symbol}*
{confidence_icon} Confidence: *{s.confidence}*

📊 *Entry Zone*: {entry_str}
🛑 *Stop Loss*:  `{_fmt(s.stop_loss, sym)}`
- *TP1*:  `{_fmt(s.tp1, sym)}` _(1:1)_
- *TP2*:  `{_fmt(s.tp2, sym)}` _(1:2)_
{tp3_line}
⚖️ *Risk/Reward*: `1:{s.rr}`
📐 *Size*:  `{s.position_size}` lots
🕐 *TF*:   {s.timeframe} | {s.confirm_tf} {s.confirm_type}
🎯 *Zone*:  {s.zone_type.replace("_", " ").title()}
